    ...
    watch(
      ...
      () => {
        ...
        let title = props.isZhCN
          ? 'Design Vue - 一套业务级 Vue 组件库'
          : 'Design Vue — An enterprise-class UI components based on Ant Design and Vue.js';
        if (item && item.title) {
          title = props.isZhCN
            ? `${item.subtitle || ''} ${item.title} - Design Vue`
            : `${item.enTitle || item.title} - Design Vue`;
        }
